...
def _make_thumb(raw_path, title, channel, duration_text, views_text, cache_path):
    ...
    bdraw.rounded_rectangle(
        [
            CARD_L - BORDER_SIZE,
            CARD_T - BORDER_SIZE,
            CARD_L + CARD_W + BORDER_SIZE,
            CARD_T + CARD_H + BORDER_SIZE
        ],
        radius=22 + BORDER_SIZE,
    ...
    draw.text((RX, 380), f"Views: {views_text}",
              font=f_sub, fill=(180,180,180))

    # Progress bar
    BAR_Y = 480
    draw.rectangle([RX, BAR_Y, RX+600, BAR_Y+6], fill=(80,80,80))
    draw.rectangle([RX, BAR_Y, RX+300, BAR_Y+6], fill=(255,255,255))

    # Time
    draw.text((RX, 500), "01:20", font=f_time, fill=(180,180,180))
    draw.text((RX+500, 500), duration_text, font=f_time, fill=(180,180,180))
    ...
    draw.text((x+2, y+2), brand, font=f_brand, fill=(0,0,0,120))
    draw.text((x, y), brand, font=f_brand, fill=(255,255,255,180))
...
async def get_thumb(videoid: str, user_id=None):
    ...
    try:
        ...
        thumb = data.get("thumbnails",[{}])[-1].get("url", YOUTUBE_IMG_URL)
        duration = data.get("duration","0:00")
        channel  = data.get("channel",{}).get("name","YouTube")
        views    = _clean_views_public(data.get("viewCount",{}).get("short","N/A"))
